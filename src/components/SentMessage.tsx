...
const SentMessage = ({data, index , pungi} : {data: any ; index: number; pungi?: boolean}) => {
  ...
  return (
    <div className={`message sent ${pungi ? "relative after:content-[''] after:color-[#d8fdd2] after:absolute after:w-0 after:h-0 after:border-solid" : ""}`} onClick={async () => {
      console.log("Clicked")
    ...
    <span className="metadata">
      <span className="time" >{date?.getHours().toString().padStart(2,  "0") + ":" + date?.getMinutes().toString().padStart(2,  "0")}</span>
      <span className="tick">
      {
                data?.status === "Sent" ? 
